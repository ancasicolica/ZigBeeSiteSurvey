...
NetworkScanRequest.prototype.rapidConnectHandling = function (callback) {
  ...
  var dataHandler = function (data) {
    ...
    if (data[1] === DIAGNOSTICS_HEADER && data[2] === NETWORK_SCAN_RESPONSE) {
      ...
      network.panId = data[5] + data[6] * 256;
      ...
      if (data[4] === 0x01) {
        ...
        if (self.options.panId) {
          ...
          foundNetworks = _.filter(foundNetworks, function (n) {
            return (network.panId === parseInt(self.options.panId, 16));
          });
          if (foundNetworks.length === 0) {
            // But return at least one with minimal data
            foundNetworks.push({panId: self.options.panId, channel: self.options.channel, rssi: -128, lqi: 0});
          }
        }
