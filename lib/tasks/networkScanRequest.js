...
/**
 * Helper: handle duplicate networks found
...
function handleDuplicates(networks) {
  ...
  for (var i = 0; i < networks.length; i++) {
    ...
    if (n) {
      // Already in list: set average (yes, I know, if the device is more than twice in the list, this is very inacurate
      // but we just want an idea about the value, we are not in medicine business)
      logger.debug('Duplicate network found ' +  networks[i].extendedPanId);
      n.rssi = Math.ceil((n.rssi + networks[i].rssi) / 2);
      n.lqi = Math.ceil((n.lqi + networks[i].lqi) / 2);
    }
    else {
