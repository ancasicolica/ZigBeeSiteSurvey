...
NetworkScanRequest.prototype.rapidConnectHandling = function (callback) {
  ...
  if (!rapidConnector.isConnected()) {
    console.log('not connected, scanning not possible');
  ...
  function networkScanResponseListener(data) {
    ...
    try {
      ...
      console.log('Exception in networkScanResponseListener');
  ...
  function scanFinishedHandler(data) {
    ...
    try {
      ...
      if (data[0] === 0x01) {
        ...
        if (self.options.panId) {
          ...
          if (self.foundNetworks.length === 0) {
            ...
            console.log('Network not found, adding empty data');
      ...
      console.log('Exception in scanFinishedHandler, try to recover');
      console.error(util.inspect(e));
      callback(e);
    }
  }
...
module.exports = {
  start: function (options, callback) {
    console.log ('New Network Scan Request');
